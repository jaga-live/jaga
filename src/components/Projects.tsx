...
const categorizeTechnologies = (technologies: string[]) => {
  ...
  return Object.fromEntries(Object.entries(categories).filter(([_key, techs]) => techs.length > 0));
};

// Extracted ProjectCard component to handle local hover states
const ProjectCard = ({ project, index, colors, navigate, setSelectedProject }: any) => {
  ...
  return (
    <motion.div
      ...
      onClick={() => {
        if (project.id === 'flantic') {
          navigate(`/projects/${project.id}`);
        } else if (project.githubUrl) {
          window.open(project.githubUrl, '_blank');
        }
      }}
      className="relative bg-white/5 rounded-xl overflow-hidden border border-white/10 hover:border-white/20 group cursor-pointer will-change-transform"
      style={{ contain: 'layout style paint' }}
    >
      {/* Tooltip Badge - Hidden when hovering interactive elements */}
      <div
        className={`absolute top-4 right-4 z-50 pointer-events-none transition-all duration-300 ${
          isHoveringInteractive
            ? 'opacity-0 translate-y-2'
            : 'opacity-0 group-hover:opacity-100 translate-y-0'
        }`}
      >
        <div className="px-3 py-1.5 bg-black/80 backdrop-blur-md border border-white/10 text-white text-xs font-medium rounded-full shadow-xl flex items-center gap-2">
          {project.id === 'flantic' ? (
            <>
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center">
          ...
          {project.id === 'flantic' && (
            <img src={flanticLogo} alt="Flantic Logo" className="w-9 h-9 object-contain ml-1" />
          )}
          {project.id === 'nescord' && (
            <img
        ...
        <p className="text-gray-300 text-sm leading-relaxed min-h-[60px]">{project.description}</p>
...
const Projects = () => {
  ...
  // Return statement uses the extracted component
  return (
    <section id="projects" className="min-h-screen py-20 px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {projects.map((project, index) => (
            <ProjectCard
              ...
              project={project}
              index={index}
              colors={projectColors[index % projectColors.length]}
              navigate={navigate}
      ...
      <AnimatePresence>
        {selectedProject && (
          ...
          >
            {/* Modal Content - same as before but using the state from this component context */}
            <motion.div
            ...
            >
              {/* ... Modal content is effectively the same, see below ... */}
              {(() => {
                  ...
                  <>
                    <div className="relative z-10 p-8 pb-8 flex-shrink-0 overflow-hidden">
                      ...
                      <div className="relative z-10 flex justify-between items-start mb-8">
                        <div>
                          <h3 className="text-4xl font-bold text-white mb-2 font-outfit tracking-tight">
                            {project.title}
                          </h3>
                          <p className="text-gray-400 text-lg">Technical Deep Dive</p>
                        </div>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
