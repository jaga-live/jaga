...
const AnimatedBackground = () => {
  ...
  const interpolateColor = (progress: number, blobIndex: number) => {
    ...
    const nextIdx = Math.min(currentIdx + 1, sectionGradients.length - 1);
    const factor = progress - currentIdx;

    const currentColor = sectionGradients[currentIdx][blobIndex];
    const nextColor = sectionGradients[nextIdx][blobIndex];
    ...
    const curr = parseRgba(currentColor);
