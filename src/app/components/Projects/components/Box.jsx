...
export default function Box({
  ...
  return (
    ...
    >
      ...
      <div
        className={`bg-white rounded-t-3xl overflow-hidden rounded-b-xl h-[300px] lg:h-[150px]  w-full bg-cover bg-no-repeat ${styles.box_img}`}
      ></div>
      <div className="mt-6 font-code">
        <span className="text-3xl font-bold lg:text-2xl sm:text-xl">
          {name}
        </span>
        {subtitle && <span className="text-xs"> ({subtitle})</span>}
        <br />
        <label className="text-xs sm:text-[10px] bg-white dark:bg-black text-[#7a7a7a] dark:text-[#C9C9C9]">
          {description}
        </label>
      ...
      <div className="dark:bg-black lg:invisible bg-white px-1 text-[10px] gap-2 flex items-center rounded-full absolute bottom-3 right-5">
        {t("status")}: {isOnline ? "online" : "offline"}{" "}
        <div
          style={{ backgroundColor: isOnline ? "#34d399" : " #6b7280" }}
