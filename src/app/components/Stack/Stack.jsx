...
export default function Stack() {
  ...
  return (
    <section className="relative scroll-mt-52" id="stack">
      ...
      <div className="grid grid-rows-2 gap-2  grid-cols-7 md:grid-cols-4 xs:grid-cols-3 xs:grid-rows-5 md:grid-rows-4 md:px-2 my-[80px]">
        <div className="col-span-3 p-4 md:col-span-4 xs:col-span-3 font-light text-2xl 2xl:text-xl xs:text-sm   min-h-24 xs:min-h-14 bg-black/5 border-black/20 dark:bg-[#0b0b0b65] dark:border-[hsl(0,0%,9%)] border-[1px] rounded-3xl relative">
          <div className="absolute md:hidden top-[14px] left-[17px] flex items-center gap-1">
            <div className="px-6 bg-black rounded-full size-2 md:px-9 xs:px-6 dark:bg-white"></div>
            <div className="px-3 rounded-full size-2 md:px-6 xs:px-3 bg-black/50 dark:bg-white/50"></div>
            <div className="px-1 rounded-full size-2 bg-black/20 dark:bg-white/20"></div>
          </div>
          {stack && (
            <span
              style={{ outlineColor: stack && stack?.color }}
            ...
            >
              ...
              <span>
                {!stack && t("coding")}{" "}
                {(locale !== "pt-BR" || stack) && "exp."}
              </span>
            </span>
          )}
          <h2 className="relative mt-3 leading-snug md:m-0 md:text-center xs:text-wrap text-nowrap dark:mix-blend-difference">
            {t("solving")}
            <br />
            {!stack && <span className="animate-text-slide">{t("stack")}</span>}
            {stack && (
              <>
                <span
                  style={{ color: stack.color }}
                  className="inline-block font-bold animate-text-slide"
                >
                  {stack.name}
                </span>
              </>
        ...
        <div className="flex items-center px-3 transition-all rounded-3xl md:hidden bg-galaxy-gradient xxs:hidden md:saturate-100">
          <Image
        ...
        {Object.keys(skills_imgs).map((skill) => (
          <div
            key={skill}
            style={{ backgroundImage: `url(${skills_imgs[skill].image})` }}
            onMouseLeave={() => handleStackLeave()}
            onMouseOver={() => handleStackHover(skills_imgs[skill])}
