...
export const projects: Project[] = [
  {
    ...
    details: {
      ...
      architecture: {
        overview: 'Flantic leverages a cutting-edge microservices architecture designed for extreme scalability and zero-downtime deployments. The system is separated into specialized services that communicate via gRPC, enabling independent scaling and updates without affecting the entire system.',
        benefits: [
        ...
        components: [
          {
            ...
            tech: ['TypeScript', 'discord-hybrid-sharding', 'gRPC', 'WebSocket']
          },
          {
            name: 'flantic-main',
            description: 'Core business logic service that processes all bot operations. Receives events from flantic-ws via gRPC and HTTP events from Discord interactions. Handles command processing, auto-moderation, custom features, and scheduled cron jobs.',
            features: [
            ...
            tech: ['NestJS', 'TypeScript', 'gRPC', 'MongoDB', 'Redis', 'PostgreSQL']
          },
          {
            name: 'flantic-rest',
            description: 'Dedicated REST API service that handles all outbound Discord API calls. Receives requests from flantic-main via gRPC and uses `@discordeno/rest` for intelligent rate limit management and request queuing.',
            features: [
          ...
          {
            name: 'flantic-config',
            description: 'Infrastructure-as-code repository containing Kubernetes configurations using Kustomize. Manages deployment configurations for multiple environments (dev, beta, prod) and supports custom bot hosting with environment-specific overlays.',
            features: [
